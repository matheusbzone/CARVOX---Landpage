...
Sempre chame a ferramenta, depois fale "Anotado, preenchi aqui na sua tela."`,
          tools: [{
            functionDeclarations: [
              ...
              {
                name: "salvar_car",
                description: "OBRIGATÓRIO: Salva TODOS os dados coletados do Cadastro Ambiental Rural (CAR) no banco de dados. Deve ser chamado no FINAL do atendimento, ou quando o produtor pedir para salvar.",
                parameters: {
                  type: "OBJECT",
                  properties: {
                    proprietario: {
                      type: "OBJECT",
                      properties: {
                        nome: { type: "STRING", description: "Nome do produtor (você já sabe)" },
                        cpf: { type: "STRING", description: "CPF do produtor (você já sabe)" },
              ...
              if (geminiMsg.toolCall && geminiMsg.toolCall.functionCalls) {
                geminiMsg.toolCall.functionCalls.forEach((funcCall: any) => {
                  ...
                  if (funcCall.name === "atualizar_formulario_ao_vivo") {
                    ...
                    addSystemMessage("Salvando dados do CAR no Supabase...");
                    fetch('/api/saveCar', {
                      method: 'POST',
