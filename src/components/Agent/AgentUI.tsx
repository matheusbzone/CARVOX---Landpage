    ...
    <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-md flex flex-col xl:flex-row items-center xl:justify-center p-4 sm:p-6 gap-6 sm:gap-8 select-none font-sans overflow-y-auto custom-scrollbar" onClick={onClose}>
              ...
              {Object.keys(liveFormData).length === 0 ? (
                ...
                <>
                  <AnimatePresence>
                    ...
                    {/* DEBUG RAW DATA */}
                    {Object.keys(liveFormData).length > 0 && !liveFormData.nomePropriedade && !liveFormData.areaTotal && !liveFormData.usoSolo && !liveFormData.possuiRio && !liveFormData.reservaLegal && (
                      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-red-900/30 border border-red-500/30 p-3 rounded-2xl overflow-hidden">
                        <p className="text-[10px] text-red-400 uppercase tracking-wider mb-1">Debug (Campos não reconhecidos)</p>
                        <pre className="text-white text-xs whitespace-pre-wrap font-mono">{JSON.stringify(liveFormData, null, 2)}</pre>
                      </motion.div>
                    )}
                    
                    {liveFormData._debugRawArgs && (
                      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-blue-900/30 border border-blue-500/30 p-3 rounded-2xl overflow-hidden mt-2">
                        <p className="text-[10px] text-blue-400 uppercase tracking-wider mb-1">DEBUG: Raw Tool Args</p>
                        <pre className="text-white text-[10px] whitespace-pre-wrap font-mono">{liveFormData._debugRawArgs}</pre>
                      </motion.div>
                    )}
                  </AnimatePresence>
                </>
      ...
      <div id="phone-container" onClick={(e) => e.stopPropagation()} className="w-full max-w-[375px] bg-[#141414] text-white rounded-[44px] p-4 shadow-[0_0_60px_rgba(0,0,0,0.8),0_0_30px_rgba(102,187,106,0.1)] relative border-[10px] border-neutral-800 overflow-hidden aspect-[9/19] flex flex-col justify-between min-h-[670px] max-h-[90vh]">
        ...
        <div className="text-center pt-2 space-y-1 relative z-10">
          ...
          {lastFunctionCall && (
            <div className="text-[10px] text-blue-400 mt-2 px-2 leading-tight bg-blue-900/20 py-1 rounded border border-blue-500/20 animate-pulse">
              ⚙️ Executando: {lastFunctionCall}
            </div>
          )}
        </div>
        ...
        {/* 4. Subtitles (Transcript) */}
        {status === "connected" && transcripts.length > 0 && (
          <div className="mx-2 mb-3 bg-black/40 border border-white/5 rounded-2xl p-3 text-center min-h-[56px] flex flex-col items-center justify-center shadow-inner gap-2">
            ...
            {/* System Messages (Supabase Status) */}
            {transcripts.filter(t => t.sender === "system").slice(-1)[0] && (
              <p className="text-[9px] text-gray-400 bg-white/5 px-2 py-1 rounded-full">
                {transcripts.filter(t => t.sender === "system").slice(-1)[0].text}
              </p>
            )}
          </div>
        )}
