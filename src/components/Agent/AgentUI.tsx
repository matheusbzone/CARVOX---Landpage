...
export default function AgentUI({ onClose }: AgentUIProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 sm:p-6 select-none font-sans" onClick={onClose}>
      <button className="absolute top-6 right-6 text-white hover:text-red-400 z-[110] transition-colors" onClick={onClose}>
        <X size={32} />
      </button>
      ...
      <div id="phone-container" onClick={(e) => e.stopPropagation()} className="w-full max-w-[375px] bg-[#141414] text-white rounded-[44px] p-4 shadow-[0_0_60px_rgba(0,0,0,0.8),0_0_30px_rgba(102,187,106,0.1)] relative border-[10px] border-neutral-800 overflow-hidden aspect-[9/19] flex flex-col justify-between min-h-[670px] max-h-[90vh]">
        ...
        <div className="text-center pt-2 space-y-1 relative z-10">
          <span className="inline-block px-2.5 py-0.5 bg-[#1B5E20]/30 text-[#66BB6A] rounded-full text-[9px] font-bold tracking-[0.15em] uppercase border border-[#2E7D32]/30">
            Gemini Flash Live • AgroCAR IA
          </span>
          <h2 className="text-2xl font-light text-white tracking-tight flex items-center justify-center gap-2 mt-2">
            🌿 <span className="font-semibold">AgroCAR</span> <span className="text-[#66BB6A]">IA</span>
          </h2>
        ...
        <div className="space-y-4 pb-4 relative z-10">
          {/* Action buttons (Mute, Keyboard, Speaker status) */}
          <div className="flex justify-around items-center px-4">
            ...
            <button
              ...
              className={`w-12 h-12 rounded-full border border-white/10 flex flex-col items-center justify-center transition-all duration-300 ${
                status !== "connected"
            ...
            >
              ...
              <span className="text-[8px] mt-1 font-bold uppercase tracking-wider text-gray-400">Mudo</span>
            </button>
            ...
            <button
              ...
              className={`w-12 h-12 rounded-full border border-white/10 flex flex-col items-center justify-center transition-all duration-300 ${
                status !== "connected"
                  ? "bg-white/5 text-white/20 border-white/5 cursor-not-allowed"
                  : showKeyboard
                  ? "bg-[#2E7D32] border-[#2E7D32] text-white shadow-[0_0_15px_rgba(46,125,50,0.4)]"
                  : "bg-white/5 text-gray-300 hover:bg-white/10 cursor-pointer"
              }`}
              title="Digitar mensagem"
            >
              <MessageSquare size={16} />
              <span className="text-[8px] mt-1 font-bold uppercase tracking-wider text-gray-400">Teclado</span>
            </button>

            {/* Speaker indicator (Shows volume response) */}
            <div className="w-12 h-12 rounded-full border border-white/10 flex flex-col items-center justify-center bg-white/5 text-gray-300">
              <Volume2 size={16} className={mode === "responding" ? "animate-bounce text-[#66BB6A]" : "text-gray-400"} />
              <span className="text-[8px] mt-1 font-bold uppercase tracking-wider text-gray-400">Som</span>
            </div>
