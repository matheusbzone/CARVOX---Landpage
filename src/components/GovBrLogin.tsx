...
import GovBrLoginModal from './GovBrLoginModal';
...
export default function GovBrLogin() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (!clientId) {
        // Modo Hackathon: Abrir o Modal Visual que simula o gov.br
        setIsLoading(false);
        setShowModal(true);
        return;
      }
  ...
  return (
    <>
      ...
      {showModal && <GovBrLoginModal onClose={() => setShowModal(false)} />}
    </>
  );
}
